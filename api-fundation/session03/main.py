...
app = FastAPI(
    title='API do Mestre da computraria',
    version='0.0.1',
    description='Uma API para estudos'
)

@app.get('/cursos', 
        description='Retorna todos os cursos de uma lista vazia.', 
...
@app.get('/calculadora')
async def calcular(a: int = Query(gt=5), b: int = Query(default = None, gt=10), x_geek: str = Header(default = None), c: Optional[int] = None, db: Any = Depends(fake_db)):
    soma: int = a + b
    if c:
        soma += c
